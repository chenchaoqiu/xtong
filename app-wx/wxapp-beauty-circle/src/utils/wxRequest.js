...
const url = 'http://t.mall.mobile.ivymei.com/wxCircle/'
...
async function getSession () {
  ...
  let {data} = await wxRequest({query},url+'api/common/autoLogin');
...
const wxRequest = async(params = {}, url,isLoading = true) => {
  ...
  if (data.imei_error_code == 0) {
    ...
    var showModal = false;
    if(url.indexOf('api/common/selfRegisterLogin') > 0){
      showModal = true;
    } else if(url.indexOf('api/common/getVerifyCode') > 0){
      showModal = true;
    }
